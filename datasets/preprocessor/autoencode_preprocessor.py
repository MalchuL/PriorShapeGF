...
from preparation.configs.surface_config import get_surface_sampling_config
...
logging.basicConfig(filename="preprocess_autoencode.log", level=logging.INFO,  format='%(asctime)s %(levelname)-8s %(message)s', datefmt='%Y-%m-%d %H:%M:%S')
...
def download_data(input_dir, output_dir, timeout, regenerate=True, include_olny=None):
    ...
    config = get_surface_sampling_config(SAMPLE_POINTS_COUNT)
...
if __name__ == '__main__':
    ...
    output_path = 'data/new_shape_net_only_points'
    TIME = 6*60 #
