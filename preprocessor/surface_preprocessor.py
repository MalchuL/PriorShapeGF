...
from datasets.configs.surface_config import get_point_sampling_config
...
def download_data(input_dir, output_dir, timeout, regenerate=True, include_olny=None):
    print('download_data')
    config = get_point_sampling_config(SAMPLE_POINTS_COUNT)
...
if __name__ == '__main__':
    path_to_dataset = '/media/malchul/Новый том/Deep_Learning/datasets/ShapeNetCore.v2'
    output_path = '/media/malchul/Новый том/Deep_Learning/datasets/shape_net_nearest_points_in_square/04554684'
    TIME = 6 * 60  #
    logging.info(f"Split data to {SAMPLE_POINTS_COUNT} points in {PARTITIONS} by using {THREAD_PARTITIONS} thread")
    download_data(path_to_dataset, output_path, TIME, regenerate=False, include_olny=['04554684'])
